...
class RequestHandle(unittest.TestCase):
    def setUp(self):
        ...
        log_level_option = DEBUG_OPTIONS['log_level'].upper()
    ...
    @staticmethod
    def get(url):
        absolute_url = HTTP_OPTIONS['endpoint'].rstrip('/') + '/' + url.lstrip('/')
        logger.debug('>>> Request URL: %s' % absolute_url)
        logger.debug('>>> Request Method: GET')
        if HTTP_OPTIONS['endpoint'].startswith('https'):#请求格式为https
            res = requests.get(
                absolute_url,
                cert=(HTTP_OPTIONS['client_cert_path'], HTTP_OPTIONS['client_key_path']),
                verify=HTTP_OPTIONS['should_verify_server_cert'])
        else:
    ...
    def post(url, data=None, json=None, **kwargs):
        ...
        if json:
            ...
            logger.debug('>>> Request MI: \r\n None')
        if HTTP_OPTIONS['endpoint'].startswith('https'):
            res = requests.post(
                absolute_url,
                json=json,
                data=data,
